...
```{r setup}
# NOTE: to be replaced with package data
...
state <- array(
  0.0, c(N_AGE_GROUPS, N_EPI_COMPARTMENTS, N_ECON_STRATA)
)
...
Note that the model allows for worker-to-worker contacts to be passed to `default_parameters()` as the named argument `"contacts_between_sectors"`, but the default behaviour is to assume there are no contacts between sectors.
...
```{r plot_epidemic}
ggplot(
  data[data$compartment == "infect_symp", ]
) +
  geom_line(
