...
```{r run_model}
# simulates a Covid-19 wild type outbreak by default
output <- daedalus("Canada")
```

Users can quickly model one of seven epidemics by passing a name from `daedalus::epidemic_names` to the `epidemic` argument.

Users can also override epidemic-specific infection parameter values as well as change the initial conditions by passing arguments to `daedalus()` as `...`.
The function documentation details which parameters are accepted.

```r
# not run
# e.g. change the transmission rate by changing r0
daedalus("Canada", r0 = 1.3) # assume r0 of 1.5, 7 days infectious period

# e.g. change the hospitalisation risk profile to model
# 1918 Influenza epidemic but with risk of severe disease
# increasing with age
daedalus(
  "Canada", epidemic = "influenza_1918",
  eta = c(2.0, 3.0, 5.0, 15.0)
)
```
...
Users can pass country data and infection parameters to override the package defaults by passing appropriately named arguments to `daedalus()` via R's `...` system.
