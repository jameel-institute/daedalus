...
_daedalus_ allows users to quickly model one of seven historical epidemics by accessing infection parameters associated with those epidemics via the `<infection>` class.
...
```{r}
# create an <infection> object to represent Covid-19 wild-type
infection_covid <- infection("sars_cov_2_pre_alpha")
```
...
infection("sars_cov_1", r0 = 2.3)

# Influenza 1918 but with mortality rising with age
infection("influenza_1918", omega = c(0.01, 0.02, 0.03, 0.1))
```
...
```{r run_model}
# simulate a Covid-19 wild type outbreak in Canada
data <- daedalus("Canada", infection_covid)
```
