...
We estimate the number of years of life lost by age and synthetic country using life tables from the @gbd_life. Formally, we used the life expectancy by age and country $\hat{L}_i$ to estimate life years lost for deceased individuals given

\begin{equation}
YLL^i = \int_{t=0}^t \sum_{j} D^{i,j}(t)dt \cdot \hat{L}_i.
\end{equation}
...
In addition to GVA losses from closures, as above, we estimate total education sector losses given

\begin{equation}
K_3 = \int_{t=0}^t \left( y_{edu}(t) + N^{i=2}(t) \cdot VSD \cdot (1 - \kappa_{edu}(t)) \cdot (1 - \pi) \right)dt
\end{equation}

where $N^{i=2}(t)$ is the number of pupils available at time $t$ (i.e., population of school-age not infected, hospitalised or deceased), $\pi$ a fixed parameter of the effectiveness of remote education, and $VSD$ is a country-specific parameter of the economic value of a school day, which we estimate as in , given by

\begin{equation}
VSD = \frac{VPE \cdot MAE \cdot RYE}{365}
\end{equation}

where $MAE$ is the mean annual earnings (estimated using GDP multiplied by labour share of income), $RYE$ the rate of return for one year of education, assumed fixed at 0.08, and $VPE$ is the yearly present value of lost earnings given by

**TODO**: Rob, could you please check how this was calculated? Is it as in p^{12} of your technical appendix? If so, could you please re-write it here, else please write down the respective equation for it. Thanks.
