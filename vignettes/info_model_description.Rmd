...
\begin{align}
    ...
    & \frac{dHr_{i,j}}{dt} = \eta^i (1 - HFR_{i}) Is_{i,j}(t) - \gamma_{Hr} H_{i,j}(t) \\
    & \frac{dHd_{i,j}}{dt} = \eta^i HFR_{i} Is_{i,j}(t) - \gamma_{Hd} Hd_{i,j}(t) \\
    & \frac{dR_{i,j}}{dt} = \zeta_{eff_{j=0}}(t) R_{i,j=0}(t) + \gamma_{Is} Is_{i,j}(t) + \gamma_{Ia} Ia_{i,j}(t) + \gamma_{Hr} Hr_{i,j}(t) - \rho R_{i,j}(t) - \zeta_{j=1} R_{i,j=1}(t)\\
...
| Hospital fatality ratio conditional on symptomatic infection leading to hospitalisation (%) | $HFR^i$ | 
