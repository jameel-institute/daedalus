    ...
    if (expected_value < 1.0) {
      ...
      auto fn_test = [idx_state, value, &i_flag = i_flag, &min_dur = min_dur,
        ...
        if (y[i_flag] < 1.0) {
          ...
          // check time_off that event has been on and return FALSE if min
          // time_off hasn't been met
          const int size_n = idx_state.size() - 1;
    ...
    for (size_t i = 0; i < n_timed_events; i++) {
      if (!ISNA(time_on[i])) {
        ...
        events.push_back(make_event(
            name_ev_time_on, {}, make_time_test(time_on[i], 0.0),
      ...
      if (!ISNA(time_off[i])) {
        ...
        events.push_back(
            make_event(name_ev_time_off, {}, make_time_test(time_off[i], 1.0),
