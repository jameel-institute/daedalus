...
- Helper function `prepare_output()` has been pruned to only work on single-group ODE solutions. The new helper function `split_multi_soln()` handles grouped ODE solutions for use in a `<daedalus_output>` object.
